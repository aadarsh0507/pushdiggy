...
const DigitalMarketingServices = () => {
  ...
  const { user } = useAuth();
  const [digitalMarketingImages, setDigitalMarketingImages] = useState([]);
  const [loading, setLoading] = useState(true);

  // Fetch digital marketing services from backend
  useEffect(() => {
    const fetchDigitalMarketingServices = async () => {
      try {
        const res = await api.get('/services');
        const digitalMarketingServices = res.data.filter(service => service.category === 'digital-marketing');
        setDigitalMarketingImages(digitalMarketingServices);
        setLoading(false);
      } catch (err) {
        console.error('Error fetching digital marketing services:', err);
        setLoading(false);
      }
    };
    fetchDigitalMarketingServices();
  }, []);
  ...
  const handleDeleteService = async (serviceId) => {
    if (window.confirm('Are you sure you want to delete this service?')) {
      try {
        ...
        const res = await api.get('/services');
        const marketingServices = res.data.filter(service => service.category === 'digital-marketing');
        setDigitalMarketingImages(marketingServices);
        setSelectedImage(null);
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (selectedImage && selectedImage._id === editingService._id) {
        const updatedService = digitalMarketingServices.find(service => service._id === editingService._id);
        if (updatedService) {
  ...
  const handleAddImage = async (e) => {
    ...
    try {
      ...
      formData.append('features', 'Digital Marketing Services');
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <div className="bg-gradient-to-br from-pink-600 to-purple-600 text-white py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold">Digital Marketing Services</h1>
                <p className="text-pink-100">Strategic digital marketing solutions for business growth</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">Add New Digital Marketing Image</h3>
              <button
            ...
            <form onSubmit={handleAddImage} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="4"
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="Detailed description of the digital marketing service..."
                />
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden relative shadow-2xl">
            ...
            <div className="flex flex-col lg:flex-row">
                    ...
                    <div className="h-full overflow-y-auto">
                      {selectedImage.images.map((image, index) => (
                        <div key={index} className="mb-6">
                          ...
                          />
                          {image.title && (
                            <h4 className="text-xl font-semibold text-gray-900 mb-2">{image.title}</h4>
                          )}
                          {image.description && (
                            <div className="text-gray-700 leading-relaxed text-base mb-4 whitespace-pre-wrap">
                              {image.description}
                            </div>
                          )}
                          ...
                          {index < selectedImage.images.length - 1 && (
              ...
              {/* Description Section */}
              <div className="lg:w-1/3 p-8 bg-gradient-to-br from-pink-50 to-purple-50 overflow-y-auto max-h-[80vh]">
                <div className="mb-6">
                  <h3 className="text-3xl font-bold text-gray-900 mb-4">{selectedImage.name}</h3>
                  <div className="w-16 h-1 bg-pink-600 rounded-full mb-6"></div>
                </div>
                
                <div className="space-y-6">
                  <div>
                    <h4 className="text-lg font-semibold text-gray-900 mb-3">Service Description</h4>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-pink-600">{selectedImage.price}</p>
                    </div>
                    <div className="bg-pink-100 px-3 py-1 rounded-full">
                      <span className="text-pink-800 text-sm font-medium">{selectedImage.category}</span>
                    </div>
                  </div>
                  ...
                  {selectedImage.features && (
                    <div>
                      ...
                      <p className="text-gray-600 text-base">{selectedImage.features}</p>
                    </div>
                  )}
                  ...
                  <div className="pt-4">
                    <button
                      ...
                      className="w-full bg-gradient-to-r from-pink-600 to-purple-600 text-white py-3 px-6 rounded-lg font-semibold hover:from-pink-700 hover:to-purple-700 transition-all duration-300 shadow-lg"
                    >
                      Get Free Quote
                    </button>
                  </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Click on any marketing service to view detailed specifications and features.
          </p>
        </div>

                {loading ? (
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">Loading digital marketing services...</p>
          </div>
        ) : digitalMarketingImages.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {digitalMarketingImages.map((marketing) => (
              <div
                key={marketing._id}
              ...
              >
                <div 
                  ...
                  onClick={() => handleImageClick(marketing)}
                >
                  {marketing.images && marketing.images.length > 0 ? (
                    <img
                      src={marketing.images[0].url}
                      alt={marketing.name}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                    <h3 className="text-white text-lg font-bold">{marketing.name}</h3>
                  </div>
                </div>
                ...
                {user && user.role === 'admin' && (
                  <div className="absolute top-2 right-2 flex gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleEditService(marketing);
                      }}
                      className="bg-pink-600 text-white px-3 py-1 rounded-lg text-xs hover:bg-pink-700 transition-colors shadow-lg"
                    >
                      Edit
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDeleteService(marketing._id);
                      }}
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">No digital marketing services available at the moment.</p>
          </div>
        )}
        ...
        <div className="mt-16 text-center">
          <div className="bg-white rounded-lg shadow-lg p-8">
            ...
            <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
              Our experts can develop a comprehensive digital marketing strategy tailored to your business goals.
            </p>
            <div className="flex justify-center space-x-4">
              ...
              <button
                onClick={() => navigate('/services')}
                className="border-2 border-pink-600 text-pink-600 px-6 py-3 rounded-lg font-semibold hover:bg-pink-50 transition-colors"
              >
                View All Services
              </button>
            </div>
