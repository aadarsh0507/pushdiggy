...
const CameraServices = () => {
  ...
  // Fetch camera services from backend
  useEffect(() => {
    const fetchCameraServices = async () => {
      try {
        const res = await api.get('/services');
        console.log('All services from backend:', res.data);
        const cameraServices = res.data.filter(service => service.category === 'camera');
        console.log('Filtered camera services:', cameraServices);
        setCameraImages(cameraServices);
        setLoading(false);
      } catch (err) {
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      if (selectedImage && selectedImage._id === editingService._id) {
        const updatedService = cameraServices.find(service => service._id === editingService._id);
        if (updatedService) {
  ...
  const handleAddImage = async (e) => {
    ...
    if (!newImage.name || !newImage.description || !newImage.image) {
    ...
    if (!allowedTypes.includes(newImage.image.type)) {
    ...
    try {
      ...
      formData.append('features', 'Camera Services');
      formData.append('category', 'camera');
      ...
      const res = await api.get('/services');
      const cameraServices = res.data.filter(service => service.category === 'camera');
      console.log('Fetched camera services:', cameraServices);
      setCameraImages(cameraServices);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold">Camera Services</h1>
                <p className="text-blue-100">Professional surveillance and security solutions</p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-gray-900">Add New Camera Image</h3>
              <button
            ...
            <form onSubmit={handleAddImage} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Security Camera System"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows="4"
                  className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                  placeholder="Detailed description of the camera system..."
                />
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden relative shadow-2xl">
            ...
            <div className="flex flex-col lg:flex-row">
                          ...
                          {user && user.role === 'admin' && (
                            <div className="flex gap-2 mt-3">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleEditService(selectedImage);
                                }}
                              ...
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleDeleteService(selectedImage._id);
                                }}
              ...
              {/* Description Section */}
              <div className="lg:w-1/2 p-8 bg-gradient-to-br from-blue-50 to-indigo-50 overflow-y-auto max-h-[80vh]">
                <div className="mb-6">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-600 max-w-2xl mx-auto">
            Click on any camera image to view detailed specifications and features.
          </p>
                  </div>

        {console.log('Rendering with loading:', loading, 'cameraImages:', cameraImages)}
        {loading ? (
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">Loading camera services...</p>
          </div>
        ) : cameraImages.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {cameraImages.map((camera) => (
              <div
                key={camera._id}
              ...
              >
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                    <h3 className="text-white text-lg font-bold">{camera.name}</h3>
                  </div>
                </div>
                ...
                {user && user.role === 'admin' && (
                  <div className="absolute top-2 right-2 flex gap-2">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleEditService(camera);
                      }}
                      className="bg-blue-600 text-white px-3 py-1 rounded-lg text-xs hover:bg-blue-700 transition-colors shadow-lg"
                    >
                      Edit
                    </button>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        handleDeleteService(camera._id);
                      }}
          ...
          <div className="text-center py-12">
            <p className="text-gray-600 text-lg">No camera services available at the moment.</p>
          </div>
        )}
        ...
        <div className="mt-16 text-center">
          <div className="bg-white rounded-lg shadow-lg p-8">
            <h3 className="text-2xl font-bold text-gray-900 mb-4">
              Need a Custom Camera Solution?
            </h3>
            <p className="text-gray-600 mb-6 max-w-2xl mx-auto">
              Our experts can design and install a custom camera system tailored to your specific security requirements.
            </p>
